submit_client_command_and_wait(sm::Sim, command) = Raw.b3SubmitClientCommandAndWaitStatus(sm, command)

function submit_client_command_and_wait_status_checked(sm::Sim, command; checked_status)
...
function set_color(sm::Sim, color; body_id, link_id=-1, shape_id=-1)
